...
public static class SimplePathfinder
{
    public static int3[] GetPath(NativeArray<float> data, int res, int3 start, int3 end)
    {
        ...
        job.start = start;
        job.end = end;
        job.openSet = new PriorityQueueSimple(Allocator.TempJob, 2048);
    ...
    private struct Job : IJob
    {
        ...
        public PriorityQueueSimple openSet;
        ...
        public void Execute()
        {
            ...
            int step = res * res;
            set[start.x * step + start.y * res + start.z] = new Node(start, 0, 0) { previous = start.x * step + start.y * res + start.z };
            ...
            while (openSet.Length != 0)
            {
                ...
                if (current.state == Node.State.Closed) { continue; }
                ...
                for (int j = 0; j < 6; j++)
                {
                    ...
                    //if (neighbour.state == Node.State.Closed && cost < neighbour.g) { log[0] = log[0] + 1000000; }
                    if (neighbour.state == Node.State.None || (neighbour.state == Node.State.Open && cost < neighbour.g))
                    {
                        neighbour.state = Node.State.Open;
                        neighbour.previous = currentIndex;
                        neighbour.g = cost;
                        neighbour.h = UtilsAI.HeuristicGridSimple(i, end) * 2;
